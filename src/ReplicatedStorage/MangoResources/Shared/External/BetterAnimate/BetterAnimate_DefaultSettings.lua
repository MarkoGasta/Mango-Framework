...
local function SimpleStateWrapper(Function)
...
local Module = {	
	...
	_State = {
		...
		Functions = {
			[Enum.HumanoidStateType.Jumping.Name] = SimpleStateWrapper(function(self)
				self:PlayClassAnimation("Jump")
				self._Time.Jumped = os.clock()
			end),

			[Enum.HumanoidStateType.Freefall.Name] = SimpleStateWrapper(function(self)
				if os.clock() - (self._Time.Jumped or os.clock()) >= self.FastConfig.WaitFallOnJump then	
					self:PlayClassAnimation("Fall")
				end
			end),

			[Enum.HumanoidStateType.GettingUp.Name] = SimpleStateWrapper(function(self) 
				self:StopClassAnimation()
			end),	

			[Enum.HumanoidStateType.FallingDown.Name] = SimpleStateWrapper(function(self) 
				self:StopClassAnimation()
			end),	

			[Enum.HumanoidStateType.PlatformStanding.Name] = SimpleStateWrapper(function(self)
				self:StopClassAnimation()
			end),

			[Enum.HumanoidStateType.Running.Name] = SimpleStateWrapper(function(self)
				...
				elseif Speed >= SpeedRange.Max and math.huge > Speed then
					...
					warn("Over 9000!!!! " .. tostring(Speed)) -- Just a meme phrase (you broke something)
				end
			end),

			[Enum.HumanoidStateType.Seated.Name] = SimpleStateWrapper(function(self)
				self:PlayClassAnimation("Sit", 0.3)
			end),

			[Enum.HumanoidStateType.Swimming.Name] = SimpleStateWrapper(function(self)
				if self._Speed > 3 then self:PlayClassAnimation("Swim", 0.4) return end
				self:PlayClassAnimation("Swimidle", 0.4)
			end),

			[Enum.HumanoidStateType.Climbing.Name] = SimpleStateWrapper(function(self)
				self:PlayClassAnimation("Climb", 0.2)
			end),

			[Enum.HumanoidStateType.None.Name] = SimpleStateWrapper(function(self)
